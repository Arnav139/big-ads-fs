...
const TransactionHistory = () => {
  ...
  const [fiterTransactions, setFilterTransactions] = useState<Transaction[]>(
    []
  );
  ...
  const selectFilter = (option: any) => {
    if (option === "polygon") {
      const filtered: any = transactions.filter(
        (transaction: any) =>
          transaction?.transactionChain === "POLYGON Testnet"
      );
      setFilterTransactions(filtered);
      setSelectedFilter("Polygon");
    }
    if (option === "Diamante") {
      const filtered: any = transactions.filter(
        (transaction: any) =>
          transaction?.transactionChain === "DIAMANTE Testnet"
      );
      setFilterTransactions(filtered);
      setSelectedFilter("Diamante");
    }
  ...
  return (
    <div className="h-full w-fulllz bg-[#14191F50] border border-[#31373F46] rounded-md p-2">
      <div className="w-full flex items-center justify-between rounded-md p-2">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            <div className=" bg-gradient-to-b from-purple-600 to-black flex items-center gap-2 rounded-md px-4 py-2">
              <button
                onClick={filter}
                className="hidden md:block text-white rounded-md cursor-pointer"
              >
                {`${selectedFilter} ${
                  selectedFilter === "Polygon" || selectedFilter === "Diamante"
                    ? `(${fiterTransactions.length})`
                    : ""
                }`}
              </button>
              <button
          ...
          <button
            ...
            className="hidden md:block bg-[#000020] text-white px-4 py-2 rounded-md cursor-pointer bg-gradient-to-b from-purple-600 to-black"
          >
            Download CSV
          </button>
          <button
      ...
      <div className="w-full h-[400px] md:h-[calc(100%-70px)] overflow-auto mt-4 scroll-hidden">
        {/* Table Header */}
        <div className="text-white bg-[#14191F] w-full min-w-[1040px] flex gap-2 items-center rounded-md p-2 sticky top-0 z-10 bg-gradient-to-b from-purple-600">
...
<div className="w-full h-[calc(100%-40px)] min-w-[1040px] pt-4">
  {loading ? (
    ...
    fiterTransactions.map((item, index) => {
      // Determine the link based on the transactionChain
      const link = item.transactionChain === "POLYGON Testnet" 
      ...
      return (
        ...
        >
          <div className="w-[5%] min-w-[40px] truncate">
            <p className="text-center">{index + 1}</p>
          </div>
          <div className="w-[15%] min-w-[150px] truncate">
            <p>{item.toUser.userId}</p>
          </div>
          <div className="w-[40%] min-w-[350px] truncate">
            <p>{item.transactionHash}</p>
          </div>
          <div className="w-[10%] min-w-[100px] truncate">
            <p>{item.event.eventType}</p>
          </div>
          <div className="w-[10%] min-w-[100px] flex items-center truncate">
            <p>
